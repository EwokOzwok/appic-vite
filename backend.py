...
df = pd.read_csv('/app/api/appic_clean_2026.csv')
...
def get_recommendations():
    try:
        ...
        # Save data to CSV
        with open('/app/data/requests_log.csv', mode='a', newline='') as file:
        ...
        if user_rec_status == 1:
            ...
            # Get the set of valid APPIC numbers in the current dataset
            valid_appic_numbers = set(df['APPIC Number'].tolist())
        
            # Walk the ranked list and collect up to 5 that exist in the new data
            top_5_user_recs = []
            for appic_number, score in user_recs:
                if appic_number in valid_appic_numbers:
                    top_5_user_recs.append(appic_number)
                if len(top_5_user_recs) == 5:
                    break
        
            # Filter and apply the same program/degree/site_type criteria
